...
# ================自定义槽函数============================
    ...
    def on_btnHandle_clicked(self):  # 为界面上的btnHandle按钮设置
        for file in self.__filelist:
            df = pd.read_csv(self.__dir_path + '/' + file, sep='/', names=['ICCID_K', 'SEID', 'UID', '打印项'], header=None, dtype=object,
            ...
            outpath = os.path.join(self.__dir_path, '修复数据')
            outfile = os.path.join(outpath, file.split('.')[0] + '_修复.mca')
        ...
        msg_box = QMessageBox(QMessageBox.Information, '处理结束', '数据处理已完成')
        msg_box.exec_()
