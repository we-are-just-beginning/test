...
# ================自定义槽函数============================
    ...
    def on_btnHandle_clicked(self):  # 为界面上的btnHandle按钮设置
        df1 = pd.read_excel(self.__file_name1,sheet_name=0,header=0,skiprows=5,usecols=[0])
        df2 = pd.read_excel(self.__file_name2,sheet_name=0,header=0)
        data = df2[df2['文件名'].isin(df1['文件名'])]
        outpath = self.__file_name1.split('.')[0] + '_提取.txt'
        data.to_csv(data_path, index=False, encoding='gbk')
