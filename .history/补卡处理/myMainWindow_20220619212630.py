...
# ================自定义槽函数============================
    ...
    def on_btnHandle_clicked(self):  # 为界面上的btnHandle按钮设置
        ...
        df2 = pd.read_csv(self.__file_name, sep=' ',
                          dtype=object, encoding='utf-8', header=None, names=['ICCID'], usecols=[4])  # 读取补卡文件
        '''
        两种补卡模式;
        ...
        data_filter = []
        if df2.columns[0][:4] == 'ICCID':
            if df2.iloc[0, 0][:4] != '8986':  # 判断是否普通卡
                for iccid in df2['ICCID']:
                ...
                list_iccid = df2.applymap(lambda x: ''.join(
                    [x[k] for k in [(i + (-1) ** i) for i in range(len(x))]]))  # 字符两两倒置
                for iccid in df2['ICCID']:
                    data = data_all[data_all['ICCID'] == list_iccid]
                    data_filter.append(data)
        ...
        if os.path.isdir(folder_path):  # 判断目录是否存在
            data_filter.to_csv(folder_path + '/' + strdatetime + '_补卡.mca',
